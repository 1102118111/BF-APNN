...
if model_type in [0 ,2]:
    ...
    def loss_res(model, *args,**kwargs):
        ...
        res2 = eps**2*g_t/c + sigma0*mu*rho_x + eps*mu*g_x + sigmar*g - eps*Avg_g
        res3 = eps**2*T_t*cv-sigmar*(2*rho-a*c*T**4)
    ...
    def loss_bc(model,*args,**kwargs):
        *o, = args
        ...
        tb,xb,ub = o
        Tr_l,_,_ = model(*o)
        Tr_r,_,_ = model(tb,xb+2,ub)
        
        g_l = fn_g(tb,xb,ub)
        g_r = fn_g(tb,xb+2,ub)
        
        return (Tr_l-Tr_r),(g_l-g_r)
    @partial(jax.jit, static_argnums=(0,))
...
if model_type in [0 ,1,2]:
    ...
    def loss_bc(model, *args,**kwargs):
        *o, = args
        tb,xb,ub = o
        Tr_l,_,g_l,_ = model(*o)
        Tr_r,_,g_r,_ = model(tb,xb+2,ub)
        return (Tr_l-Tr_r),(g_l-g_r)
    @partial(jax.jit, static_argnums=(0,))
    def loss_ic(model, *args,**kwargs):
        *o,T_val = args
        Tr,T,g,_ = model(*o)
        return T_val-Tr,T_val-T,g
...
def loss_func(params,graphdef,freeze_params,Xr,Xb,Xi,Xe,config,*argv):
    ...
    loss_r,loss_r_weight,config["rsum"] = loss_ca(vmap(lambda *o: loss_res(NN,*o))(*trans_squeeze(Xr)))

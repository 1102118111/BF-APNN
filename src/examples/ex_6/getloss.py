...
left_boundary =-1.5
right_boundary = 1.5
Lb = jnp.array([0,left_boundary,left_boundary,0.,0.])
Ub = jnp.array([1,right_boundary,right_boundary,jnp.pi,2*jnp.pi])
lb = Lb[:-2]
ub = Ub[:-2]
...
if model_type in [1,2]:
    ...
    def loss_res(NN,Xr,seeds):
        ...
        _,g,_,_ = jnp.split(NN(tr,xr,yr,thetar,phir),4,1)
    ...
    @jax.jit
    def loss_bc(NN,Xb):
        td,xd,yd,ud,vd = jnp.split(Xb,5,1)
    ...
    def loss_ec(NN,Xe):
        try:
            ...
            te,xe,ye,ue,ve,val_rho = jnp.split(Xe,Xe.shape[-1],1)
            rho,*_ = jnp.split(NN(te,xe,ye,ue,ve),outdim,1)
            return rho-val_rho,
        except:
            return jnp.zeros((1,1)),
